	...
	<div class="rounded-xl border bg-white p-6 shadow-sm">
		<div class="flex items-center gap-4">
			...
			<div class="flex-1">
				...
				<span class="mt-1 inline-block rounded-full px-3 py-1 text-xs font-medium {roleColors[data.profileUser.role as keyof typeof roleColors]}">
		...
		<div class="rounded-xl border bg-white p-6 shadow-sm">
			...
			<div class="space-y-4">
				{#each data.articles as article (article.id)}
					<a href="/article/{article.id}" class="block rounded-lg border p-4 transition-all hover:shadow-md">
						<h3 class="mb-2 text-lg font-bold text-zinc-900">{article.title}</h3>
						<p class="mb-3 text-sm text-zinc-600">{article.excerpt}</p>
						<p class="text-xs text-zinc-500">Publié le {new Date(article.createdAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
					</a>
				{/each}
			</div>
		</div>
